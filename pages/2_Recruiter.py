        ...
        if st.button("Process & Rank Candidates", type="primary", use_container_width=True):
            if uploaded_resumes:
                ...
                for i, resume_file in enumerate(uploaded_resumes):
                    ...
                    if "Error" not in resume_text and resume_text.strip():
                        ...
                        st.error(f"Could not read {resume_file.name}. Skipping.")

                    progress_bar.progress((i + 1) / len(uploaded_resumes))
                
                status_text.text("Ranking complete!")
                st.success(f"Successfully processed {len(uploaded_resumes)} candidates. View results in the Vault.")
            else:
...
with tab_vault:
    ...
    if not active_jobs:
        ...
        # FIX 1: Match the dropdown labels exactly to Tab 2 so you know which campaign is which!
        vault_job_options = {f"{job['title']} ({job['created_at'][:10]})": job['id'] for job in active_jobs}
        
        # Added a unique 'key' to prevent Streamlit UI confusion
        ...
        if not candidates:
            ...
            with col1:
                candidate_to_delete = st.selectbox("Select a candidate to remove from this campaign:", list(candidate_map.keys()))
            with col2:
                st.markdown("<br>", unsafe_allow_html=True) # Adds a little spacing to align with the dropdown
                if st.button("Remove Candidate", type="secondary", use_container_width=True):
                    db.delete_candidate(candidate_map[candidate_to_delete])
                    st.toast(f"{candidate_to_delete} has been removed.")
                    st.rerun()